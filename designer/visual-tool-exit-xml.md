...
When `R00002` is not visible, it is **absent** from the galaxy slice. The visual tool regional map (`buildRegionMapCells` in `visual-tool/src/parsers/reportXml.ts`) needs **X**, **Y**, and **type** for every exit target on the same body to lay out the grid and grey out unexplored neighbours. Today it falls back to “region not in report XML” with no coordinates.
...
- [`architecture/delivery/campaign-play.md`](../architecture/delivery/campaign-play.md) — visual tool dependency pointer
