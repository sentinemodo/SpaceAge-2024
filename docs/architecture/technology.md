...
Local SonarQube helper scripts were removed; cloud CI uses NUnit via `.cursor/run-tests.sh` / Windows vstest only.
...
| RAG corpus | Hybrid prompt pack + vector chunks over `play/player/*.md` + per-faction report/story/orders | Never `gamein` / other factions / raw full `data.xml` for strategist |
...
| Phase 4 tools | `/eta`, `/battle` TypeScript islands | Port published formulas (`SpaceTransit`, `au-transit.md`, `play/player/battle.md`). No `Game.exe`, no live `data.xml` |
