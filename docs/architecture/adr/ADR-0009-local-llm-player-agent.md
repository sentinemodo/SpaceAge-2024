...
| Index | Contents |
|-------|----------|
| **Shared** | `play/player/rules.md`, `play/player/battle.md`, SampleGame or campaign tech manuals under `play/player/` (path chosen by play mode) |
| **Per-faction** | That faction’s isolated report, `story.md` / objective, prior `order.*` / drafts for that seat |
| **Never** | `gamein.xml`, `gameout*.xml`, other factions’ reports, full raw `play/campaign/data.xml` / `Tests/data.xml` for the campaign strategist path (catalog grounding stays via player manuals) |
...
3. Compact prefixes & subjects excerpt from `play/player/rules.md`.
...
- **Hybrid RAG** fits a small, structured corpus (`play/player/*.md` + reports): selective recall beats dumping the repo or fine-tuning on sparse order corpora.
...
Phased work (environment, RAG corpus prep, post-turn and engine/data refresh) is specified in [`docs/architecture/delivery/local-player-agent.md`](../delivery/local-player-agent.md). Do not implement inside `Game.exe`.
