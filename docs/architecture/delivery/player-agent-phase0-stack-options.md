...
|--------|-----|--------|------|
| Small corpus RAG (`play/player/*.md` + reports) | Adequate (`Microsoft.Extensions.AI`, `Microsoft.Extensions.VectorData`, LiteDB v6 vectors, or in-memory) | **Strongest** ecosystem (Chroma, LanceDB, FAISS) | Good (LangChain.js, LanceDB) |
...
2. **C# retrieve:** chunk `play/player/rules.md` → embed → store → `TopK` retrieve for query “MOVE order syntax”.
3. **Quality gate (required):** repeat draft with **`ai/qwen2.5-coder`** (or local Ollama `qwen3-coder:8b`) on one SampleGame report; run verb allowlist lint; compare shape to `play/player/drafts/`.
