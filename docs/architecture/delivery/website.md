...
Player-facing flavour lives in [`docs/legacy/alderson/Rules.txt`](../../docs/legacy/alderson/Rules.txt). Operational play-loop checkboxes live in [`campaign-play.md`](campaign-play.md) (owned by the play/campaign/designer track — **do not edit that file from this workstream**).
...
| Folder `website/` at repo root | Separate from `Game/`, `Tests/`, `play/campaign/`, `play/` |
...
| Dumping agent rulebook | `/rules` renders `docs/human/rules.md` at build time |
...
| Engine TDD / C# / `play/campaign/` XML | Other agents |
...
| Distance | **Two AU-from-star** numbers (origin, destination). ΔAU = \|AU₂ − AU₁\|. This is the Helios/Fomal geometry ([`play/designer/au-transit.md`](../../play/designer/au-transit.md)): Arbor/Anvil = 1.0, Gates = 80. Optional **presets** (planet→moon 0.04, belt 1.7, gas giant 4.2, Gate 79) may fill the two fields; they must not replace the two-AU model |
...
| Catalog | **User-entered stats** for MVP. A later committed UTF-8 **combat-stats excerpt** (public module attack/defense/damage/HP only — not `play/campaign/data.xml`) is optional. Never ship or fetch the live catalog |
...
**Loop** (document in the page; implement from [`play/player/battle.md`](../../play/player/battle.md), not from a C# import):
...
- [x] Visual tool **implementation** remains a different agent / folder / host (`tools/visual-tool/` + game-host `/client/`)
...
|------|------|----------------|
| **Architect** | `docs/architecture/` (this plan, ADR-0007, indexes) | `website/` source, C#, campaign XML |
...
5. Rules.txt §§1, 1.1, 2.1 for copy — plus `play/player/rules.md` for a later `/rules` link, not for flavour.
...
- Do **not** glob `docs/architecture/**` (architect-owned). The developer **reads** this file; they do not own it.
...
docs/architecture/delivery/website-scenarios.md
```
...
If the intended change **diverges** from `docs/architecture/delivery/website.md`, [ADR-0007](../adr/ADR-0007-public-campaign-website.md), or `docs/architecture/technology.md` (Website section), **stop**. Do not silently deviate. Present the gap (what architecture says vs what the request or code would do). Require **explicit human approval of this deviation** before writing the diverging code.
...
| When | Path | Role |
|------|------|------|
| **Now** (no `website/` yet) | [`docs/architecture/delivery/website-scenarios.md`](website-scenarios.md) | Seed source of truth |
