...
Related: `.cursor/agents/player.md`, `play/player/README.md`, campaign play isolation docs when present
...
play/player/                      # canonical manuals (RAG shared corpus)
docs/architecture/adr/ADR-0009-…  # decision
```
...
- [x] **`--mode test|campaign`** required on `ingest-shared`, `ingest-faction`, and `draft` (separate shared indexes; `test` = SampleGame manuals, `campaign` = `play/player/campaign/*`).
...
|--------|----------------|----------|
| `play/player/rules.md` | By verb / `##` heading | `doc=rules`, `verb=…` |
| `play/player/battle.md` | By section | `doc=battle` |
| `play/player/basic_technologies.md` + `play/player/advanced_technologies.md` | Tech / module / item entry | `doc=tech`, `mode=test` |
| `play/player/campaign/basic_technologies.md` (+ advanced when present) | Same | `doc=tech`, `mode=campaign` |
| Prior drafts under `play/player/drafts/` (optional style) | Per `#modulestack` / `#person` block | `doc=draft` |
...
- [ ] `play/player/rules.md` reflects implemented verbs only (per `/player` hard rules).
- [ ] Campaign vs SampleGame tech manuals point at the correct catalog excerpt.
- [ ] `play/player/README.md` links this plan and notes which files are RAG sources.
...
- [x] Write UTF-8 draft to `play/player/drafts/` or `play/runs/<id>/factions/NN/orders.{faction}.{turn}.{iteration}.txt`.
- [x] Reminder in README: GM / `turn.ps1` converts to Windows-1251 for `/turn-dir`.

**Done when:** One SampleGame or campaign faction report produces a lint-passing draft comparable in shape to existing `play/player/drafts/`. **Code complete (2026-09-10)** — run `ingest-shared` + `ingest-faction`, then `draft` against a live Ollama host to produce a lint-passing file; unit tests cover allowlist, lint, prompt pack, and writer normalization.
...
- `Tests/data.xml` or `play/campaign/data.xml` catalog change affecting techs/modules/items.
- Manual edit to `play/player/rules.md`, `play/player/battle.md`, or tech manuals.
