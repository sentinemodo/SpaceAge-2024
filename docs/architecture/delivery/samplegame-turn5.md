...
|--------|-----------|----------------|
| `play/player/order_wishlist.md` | `TRANSFER ALL` / pick damaged module | Drafts use `STACK OUT` + `GIVE`/`GET`. |
| `play/player/order_wishlist.md` | `REPAIR` without `CanOperate` | Captured Sydney guns are leftovers; not the space beat. |
| `play/player/technologies_wishlist.md` | Wire `USE repair` / `[medfac]` | Explicitly out since turn 4. |
| `play/designer/engine-wishlist.md` | Typed weapons, shield intercept, armour hit-bias | Too large; Battle stays flat. |
| `play/designer/engine-wishlist.md` | Item `attack`/`damage` in battle (`rctlnc`) | Infantry rockets, not orbital `[orbrkt]`. |
| `play/designer/engine-wishlist.md` | Shuttle orbit `efficiency-multiplier="10"` (50-week `orassm`) | No orbit `USE` this story; leave for a later construction beat. |
| `play/designer/engine-wishlist.md` | Shuttle launch fuel surcharge / land bans | Would block `[117]` unless extra `h2o2` is seeded; skip. |
| `play/designer/engine-wishlist.md` | Space transit ΔAU × drive speed | Luna→Earth must complete **this quarter**. Keep ETA 0/1. |
| `play/designer/engine-wishlist.md` | Moon `@name` / `loadGalaxyExits` | Hull already space-hops without matching exits. |
...
|---|--------|------------|------------|
| 0 | Docs | This checklist + `play/player/drafts/orders.5.*` | n/a |
| 1 | Catalog `[orbrkt]` | `Tests/data.xml` + `play/campaign/data.xml` | `TDataFile`: L1, production anywhere, use-time 8, size 40 (shuttle still holds crew/food/air/fuel), module operates in orbit, drones can still win |
