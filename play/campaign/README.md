...
| `_gen_gamein.py` | Generator. Run `python play/campaign/_gen_gamein.py` to rewrite `gamein.1.xml` from `play/designer/galaxy.md` |
...
Engine `LoadGame` always opens **`/data/gamein.xml`**. Play and TDD copy `play/campaign/gamein.1.xml` → run `/data/gamein.xml` and `play/campaign/data.xml` → run `/data/data.xml`. Never point `/data` at `play/campaign/` (that would write `gameout` into the catalog tree). Never put `gamein.xml` in `/turn-dir` (`/turn-dir` is `order.*` in and `report.*` out).
...
L3+ signature ores from `play/designer/resources.md` (`lithia`, `reeox`, `boron`, `grphit`, `xenon`, `nitrat`, `berylm`) are catalog items and are seeded on SS0003–SS0010. Extraction techs for those ores are in `data.xml`. `hydzn` is catalog-only (refined, not a region ore).

Environment / gravity / temperature rules: `play/designer/environments.md`. Galaxy seed (militias, Alderson Gates): `play/designer/galaxy.md`.

The **public lobby website** (human-facing home, visual-tool link, current-turn orders status) is specified under `docs/architecture/delivery/website.md` and tracked as a campaign-play todo in `docs/architecture/delivery/campaign-play.md`. It is not this catalog tree and is not the `play/runs` AI isolation path.
