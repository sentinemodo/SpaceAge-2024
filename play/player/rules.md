...
From `Game.exe` (`Program.Main`) and `Game.Execute`. A turn is **13 weeks**. Combat details: `play/player/battle.md`.
...
7. **Battles** — `Battle.StartAtLocations` then `Execute` (see `play/player/battle.md`). Then drop stale per-unit stances again.
...
**Underwater seat tender (campaign):** when `USE` produces a **settlement**-group module, an extra tender check runs (`UseOrder.HasUnderwaterTenderForSettlement`). The **seat location** (the `FOR` parent’s location, else the producer’s) must already hold the producer’s faction’s **underwater** craft (`Location.HasUnderwaterPresence`: own stack with `ModuleType.Underwater` and quantity > 0, including nested). Gate triggers when the produced settlement has catalog `underwater="yes"` (campaign: under-surface city `[uscty]`), **or** when producing small dome city `[dmdcty]` into a **liquid-surface** seat. Non-settlement underwater builds (`uwtruk`, `uwtank`, `udrill`) and pontoon city `[ptncty]` skip this gate. Surface naval (`coastr` / `gunbot`) does **not** count as tender. Failure: `USE failed: {module} requires an underwater transport or combat craft in {location}.` Solid-surface `dmdcty` seats skip this gate.

**Settlement air upkeep (campaign flavour):** pontoon city `[ptncty]` and under-surface city `[uscty]` omit `<upkeep type="terair"/>` (open / shaft air). Small dome city `[dmdcty]` always bills food + terran air `[terair]` + cash, including on liquid-surface (closed shell, no surface shaft). Underwater vehicles still **consume** `terair` as crew life support.
