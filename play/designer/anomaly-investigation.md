...
Regional **anomalies** are one-cell survey targets on the habitable grids. They are emitted as `<anomaly type="…" description="…"/>` on the **anomaly region**. Turn-1 reports from an adjacent HQ grant already append `, anomaly detected` on the matching exit (engine (live)). Full `description` stays hidden until investigation resolves the site.

Hard science only: spectra, magnetometry, passive seismics, gravimetry, gamma spectroscopy — not prophecy or psionics.

Seed list (HQ-adjacent, t=1): [`anomaly-seed.md`](anomaly-seed.md). Source ids: `play/campaign/_gen_gamein.py` (`HQ_ANOMALIES`).
...
| Artifact | Action |
|----------|--------|
| `play/designer/anomaly-investigation.md` | This doc — benefit tiers, loop, HQ hooks |
| `play/designer/engine-wishlist.md` | New investigation rows |
| `play/campaign/*` | No XML change in this pass (rename + payouts after TDD) |
