...
Unknown **attributes** are ignored. Unknown **module `group`**, **`location-type`**, or contract **`trigger`** throw on load — do not put those in `play/campaign/` until TDD lands the wishlist row.
...
The loader hardcodes filename `gamein.xml` under `/data`. The committed seed is `play/campaign/gamein.1.xml`; copy it to the run `/data/gamein.xml` and copy `play/campaign/data.xml` to `/data/data.xml`. Never point `/data` at `play/campaign/` (would write `gameout` into the catalog tree). Never put `gamein.xml` in `/turn-dir`.
...
- **Regional anomaly** — optional child `<anomaly>` on `<region>`. Required attr `type` (`spectral` \| `magnetic` \| `seismic` \| `gravimetric` \| `radiometric` \| `anomaly`). Optional attrs `description` (hard-science survey note; not shown on exit hint until investigation resolves), `points` (threshold, default **8**; HQ-adjacent minors in campaign seed use **20**). Child `<reward band="N" kind="survey-blurb|research-rp|technology|resource" …/>` defines payout bands in **gamein** (persist through save). Save-only children: `<progress faction="…" quantity="…"/>`, `<resolved faction="…"/>`. Order **`RESEARCH <region-id>`** on-site advances progress (engine (live)). Exit hint: `, anomaly detected` (engine (live)). Legacy `<poi>` loads as alias until saves migrate.
- **Deep pocket** — optional child `<deep-pocket>` on `<region>` with nested `<resource type="…" quantity="…"/>`. Subsurface ore requiring a **core drill** module to extract. Exit hint from an owned grant: `, deep pocket of resources detected` when the observer holds **`mcored` tech or core drill module in the source region** (engine (live)). **`Deep resources:`** assay line appears only when **`mcored` tech or core drill module is present in the pocket region**. See [`deep-pockets.md`](deep-pockets.md).
- **Settlement exit hint** — when a neighbouring region holds any **settlement-group** module stack (`town`, `city`, `mtrply`, dome variants), exit lines from a **visible source region** append `, settlement detected` (engine (live)). Does not name the settlement or its owner.
