...
Ten **minor** anomalies — one orthogonally adjacent cell per player HQ grant on Arbor/Anvil. Emitted as `<anomaly type="…" description="…"/>` on the **anomaly region**, not on the grant. Turn-1 region reports from the grant append `, anomaly detected` on the matching exit (engine, live).
...
Source of truth for ids: `play/campaign/_gen_gamein.py` (`HQ_ANOMALIES`). Regenerate `gamein.1.xml` after edits.
