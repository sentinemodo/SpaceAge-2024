...
Campaign design for body attributes on planets/moons. Attrs are emitted on galaxy bodies and **live** in `LoadGalaxy` / `BodyEnvironment` (live). Catalog planet/moon **types** carry default flavour only.
...
Campaign gas giants at t=1 emit **hostile** + both isotopes (see `play/campaign/_gen_gamein.py`).
