...
Campaign baseline: **every live id** from `Tests/data.xml` (player manuals `play/player/basic_technologies.md`, `play/player/advanced_technologies.md`). Do **not** rename live ids. Tech `airgen` builds module `lifsys` — there is no live tech named `lifsys`.
...
Module ids ≤ 6 characters. Live module **groups** only. New consume item ids are the closed set in `play/designer/resources.md` — do not invent an ore here without a row there.
...
| Id | Name | Requires | Use-time | Consume | Produce | Notes |
|----|------|----------|----------|---------|---------|-------|
| **`fustch`** | fusion torch drive | `he3fus` | 6 | 20 `titani`, 8 `silici`, 6 `copper`, 4 `heliu3` | `fustor` | Magnetic-nozzle He3 torch. **Fuel 2 `heliu3` / week** (thirstier than `rctdrv` 1 `h2o2` / wk). Space `speed` 1: Gate **39** wk (**78 He3**). Burn–coast–burn at ≤1.5 g. **Live in `play/campaign/data.xml`**; `f(ΔAU)` 2 / 6 / 13 / 39 |
...
Full stats: `play/designer/catalog.md`. Fuel/feedstock must match `resources.md`.
...
t=1 wrecks **L3–L6**, off Arbor/Anvil grids (`play/designer/contracts.md`).

## Closed consume set

Every item id a tech or module fuel uses: see **`play/designer/resources.md`**. Do not add a consume line here without a dictionary row and a seed location.
