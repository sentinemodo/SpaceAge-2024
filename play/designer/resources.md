...
Canonical list for campaign galaxy seeding and `play/campaign/data.xml` item entries. **Do not fork:** `play/designer/catalog.md` points here. Consume/fuel ids come from `play/designer/technology.md`. Galaxy occupancy (Arbor/Anvil split) is `play/designer/galaxy.md`; this file owns **which ids exist, rarity, environments, and quantities**.
...
| `skimmn` | 2 | `he3min` | module `ramsco` | production build |
| `he3skm` | 2 | `skimmn` | 1 `heliu3` | `atmosphere` + `gasgnt` orbit |
...
Gas-giant **orbit** `deutrm`/`heliu3` cloud resources and `location-type="atmosphere"` are **live** (engine (live)). Campaign seeds them on every `gasgnt` orbit with `atmosphere` ≠ `none`; ice-moon pockets remain for early L2–L4 without a scoop fleet.
