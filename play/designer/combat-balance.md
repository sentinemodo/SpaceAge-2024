...
Canonical thresholds for retuning `play/campaign/data.xml` military modules. Engine rules: [`play/player/battle.md`](../play/player/battle.md). Typed matchups are **live** in `CombatMatchup` / `Battle` when catalog attrs are set (campaign emits them; SampleGame stays flat).
...
## Benchmark matrices (post-retune — play/campaign/data.xml)
