...
Live catalog: `play/campaign/data.xml`. Geometry: `play/campaign/gamein.1.xml` + [galaxy.md](galaxy.md). Engine duration: `SpaceTransit.DurationWeeks` (ΔAU × drive `speed` × mass factor).

**`f(ΔAU)` is live (2026-08-29).** Default workshop-frigate ETAs **2 / 6 / 13 / 39**.
...
ΔAU = **79 AU** = 1.182×10¹³ m (IAU AU = 1.495978707×10¹¹ m). `play/campaign/data.xml` has no AU attributes (catalog only). Type `adpnt` is unused; live Gates are first-class `<alderson>`.

Same numbers on Fomal: Anvil `P00005` AU 1.0 → Fomal Gate `P00010` AU 80.

## Locked `f(ΔAU)` (live, 2026-08-29)
...
Masses use live `play/campaign/data.xml` (`sshull` 100, `cbridg` 300, `fisrec` 140, `fustor` 1400, `crwqrt` 400, `factry` 750, `cmplib` 50, `cargob` 200; `terran` 4, `food`/`terair`/`h2o2`/`heliu3` 1, `uraniu` 8, `iron` 10).
