...
**Local runner (Phase 0+):** [`tools/player-agent/`](../tools/player-agent/README.md) — C# net8 CLI (`dotnet run --project tools/player-agent/PlayerAgent.csproj -- …`). Use `--mode test|campaign`; draft with `--output` or `--run` + `--faction`. After isolate copies new reports, run [`play/ingest-rag.ps1`](../play/ingest-rag.ps1) (or `ingest-run`) before drafting; use `ingest-faction --story-only` when `/campaign-ai` updates `story.md`. After engine or catalog changes, refresh these manuals first, then run [`play/refresh-shared-rag.ps1`](../play/refresh-shared-rag.ps1) (or `refresh-shared`) before the next AI draft batch.
