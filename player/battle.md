...
**Officers / people:** skill bonuses apply only when the officer’s **root modulestack** matches the skill’s `<usable-in>` (module group / stack size). Flat catalog `attack`, `defense`, `initiative` stack on the parent; `produce effect="effective attack"` / `effective defence` with `target="units"` adds `value × QuantityActive` of that root stack when the skill applies. **`hmedic`** `cure-chance` lowers the quarterly wounded recover roll threshold stack-wide (nested officers count); death threshold unchanged. Examples: frigate pilot on a frigate hull (+5 defense, +5 initiative); armor platoon leader on vehicles (+5 attack, +5 initiative, +1 attack per active vehicle); infantry battalion commander on infantry (+5 attack, +5 initiative, +1 defense per active battalion module).
